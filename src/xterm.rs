    ...
    (
        ...
        pub struct $nom:ident {
            $(
                ...
                $(|clone(get = $get:ident, set = $set:ident, js_name = $js_name:ident $(, pub = $public:ident)?)
        ...
        impl $nom {
            ...
            //     pub fn $($set)?(&mut self, $($field: $field_ty)?) {
            //             $(self.$field = $field;)?
            //     }
            // )*
...
pub struct Theme {
    ...
    |clone(get = background, set = set_background, js_name = background)
    background: Option<Str>,

    /// ANSI black (eg. `\x1b[30m`).
    |clone(get = get_black, set = set_black, js_name = black)
    black: Option<Str>,

    /// ANSI blue (eg. `\x1b[34m`)
    |clone(get = get_blue, set = set_blue, js_name = blue)
    blue: Option<Str>,

    /// ANSI bright black (eg. `\x1b[1;30m`)
    |clone(get = get_bright_black, set = set_bright_black, js_name = brightBlack)
    bright_black: Option<Str>,

    /// ANSI bright blue (eg. `\x1b[1;34m`)
    |clone(get = get_bright_blue, set = set_bright_blue, js_name = brightBlue)
    bright_blue: Option<Str>,

    /// ANSI bright cyan (eg. `\x1b[1;36m`)
    |clone(get = get_bright_cyan, set = set_bright_cyan, js_name = brightCyan)
    bright_cyan: Option<Str>,

    /// ANSI bright green (eg. `\x1b[1;32m`)
    |clone(get = get_bright_green, set = set_bright_green, js_name = brightGreen)
    bright_green: Option<Str>,

    /// ANSI bright magenta (eg. `\x1b[1;35m`)
    |clone(get = get_bright_magenta, set = set_bright_magenta, js_name = brightMagenta)
    bright_magenta: Option<Str>,

    /// ANSI bright red (eg. `\x1b[1;31m`)
    |clone(get = get_bright_red, set = set_bright_red, js_name = brightRed)
    bright_red: Option<Str>,

    /// ANSI bright white (eg. `\x1b[1;37m`)
    |clone(get = get_bright_white, set = set_bright_white, js_name = brightWhite)
    bright_white: Option<Str>,

    /// ANSI bright yellow (eg. `\x1b[1;33m`)
    |clone(get = get_bright_yellow, set = set_bright_yellow, js_name = brightYellow)
    bright_yellow: Option<Str>,

    /// The cursor color
    |clone(get = get_cursor, set = set_cursor, js_name = cursor)
    cursor: Option<Str>,

    /// The accent color of the cursor (fg color for a block cursor)
    |clone(get = get_cursor_accent, set = set_cursor_accent, js_name = cursorAccent)
    cursor_accent: Option<Str>,

    /// ANSI cyan (eg. `\x1b[36m`)
    |clone(get = get_cyan, set = set_cyan, js_name = cyan)
    cyan: Option<Str>,

    /// The default foreground color
    |clone(get = get_foreground, set = set_foreground, js_name = foreground)
    foreground: Option<Str>,

    /// ANSI green (eg. `\x1b[32m`)
    |clone(get = get_green, set = set_green, js_name = green)
    green: Option<Str>,

    /// ANSI magenta (eg. `\x1b[35m`)
    |clone(get = get_magenta, set = set_magenta, js_name = magenta)
    magenta: Option<Str>,

    /// ANSI red (eg. `\x1b[31m`)
    |clone(get = get_red, set = set_red, js_name = red)
    red: Option<Str>,

    /// The selection background color (can be transparent)
    |clone(get = get_selection, set = set_selection, js_name = selection)
    selection: Option<Str>,

    /// ANSI white (eg. `\x1b[37m`)
    |clone(get = get_white, set = set_white, js_name = white)
    white: Option<Str>,

    /// ANSI yellow (eg. `\x1b[33m`)
    |clone(get = get_yellow, set = set_yellow, js_name = yellow)
    yellow: Option<Str>,
}}
...
//     #[wasm_bindgen(js_name = "bellSound")]
//     pub bell_sound: Option<Str>,
...
//     #[wasm_bindgen(js_name = "windowOptions")]
...
//     #[wasm_bindgen(js_name = "wordSeparator")]
//     pub word_separator: Option<Str>,
// }
...
//     fn new(options: Option<TerminalOptions>) -> Terminal;
